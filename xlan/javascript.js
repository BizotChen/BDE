...
Blockly.Arduino.i2s_media_azure_stt = function(block) {
  ...
  if (Blockly.Arduino.my_board_type=="ESP32") {
    ...
    return 'recTime = ' + time + ';\nazureTranscribedDirectly(\"' + dropdown_lang + '\", ' + key + ', ' + dropdown_punctuation + ');\n';
  } else {
    return '';
  }
};
...
Blockly.Arduino.motordriver_init = function(block) {
  Blockly.Arduino.definitions_.motordriver_include = '#include "Emakefun_MotorDriver.h"';
...
Blockly.Arduino.motordriver_attach_motor = function(block) {
  var gpio = block.getFieldValue('gpio');
  Blockly.Arduino.definitions_.motordriver_motor = 'Emakefun_DCMotor *motor_' + gpio + ' = mMotorDriver.getMotor(' + gpio + ');';
  var code = '';
...
Blockly.Arduino.motordriver_stop_motor = function(block) {
  var gpio = block.getFieldValue('gpio');
  var code = 'motor_' + gpio + '->run(BRAKE);\n';
...
Blockly.Arduino.motordriver_attach_servo = function(block) {
  var gpio = block.getFieldValue('gpio');
  Blockly.Arduino.definitions_.motordriver_servo = 'Emakefun_Servo *servo_' + gpio + ' = mMotorDriver.getServo(' + gpio + ');';
  var code = '';
